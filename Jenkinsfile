pipeline {
    environment {
        registry = "vinothramesh/blue-app"
    ...
    stages {
        stage('Cloning Git') {
            steps {
                git 'https://github.com/vinoth-ramesh/udacity-cloud-devops-capstone.git'
            }
        }
        stage('Lint') {
            steps {
        ...
        stage('Security Scan Image') {
            steps {
                aquaMicroscanner imageName: "vinothramesh/capstone-project-green-app:latest", notCompliesCmd: 'exit 1', onDisallowed: 'fail', outputFormat: 'html'
            }
        }
